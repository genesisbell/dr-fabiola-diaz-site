...
export default function AboutSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-center py-16">
      ...
      <div className="relative z-10 page-container flex flex-col md:flex-row items-center justify-center w-full">
        ...
        >
          <Image
            ...
            width={460}
            height={600}
            className="w-[360px] md:w-[460px] h-auto drop-shadow-2xl"
            priority
          />
        </motion.div>

        {/* Card + location — slides in from right */}
        <motion.div className="flex flex-col gap-3" {...slideIn("right")}>
          <div className="flex flex-col gap-4 bg-white rounded-2xl shadow-md px-10 py-8 md:pl-16">
            <h2 className="text-3xl font-bold text-gray-900">{t.general.doctorName}</h2>
            <p className="text-lg font-bold text-secondary">{t.general.specialty}</p>
          ...
          <p className="text-[3rem] font-bold text-white/80">Guadalajara, Jalisco</p>
        </motion.div>
      </div>
    </section>
  );
